...
public class LongestBalancedSubarrayII {
    ...
    public int longestBalanced(int[] nums) {
        ...
        SegmentTreeLongestBalancedSubarrayII seg =
                new SegmentTreeLongestBalancedSubarrayII(prefixSum);
    ...
    public static void main(String[] args) {
        LongestBalancedSubarrayII solver = new LongestBalancedSubarrayII();

        System.out.println(solver.longestBalanced(new int[]{2,5,4,3}));     // 4
        System.out.println(solver.longestBalanced(new int[]{3,2,2,5,4}));   // 5
        System.out.println(solver.longestBalanced(new int[]{1,2,3,2}));     // 3
        System.out.println(solver.longestBalanced(new int[]{22,36,22}));    // 0
    }
}

class LazyTagLongestBalancedSubarrayII {

    int toAdd;

    LazyTagLongestBalancedSubarrayII() {
        this.toAdd = 0;
    }

    void add(LazyTagLongestBalancedSubarrayII other) {
...
class SegmentTreeNodeLongestBalancedSubarrayII {

    int minValue;
    int maxValue;
    LazyTagLongestBalancedSubarrayII lazyTag;

    SegmentTreeNodeLongestBalancedSubarrayII() {
        this.minValue = 0;
        this.maxValue = 0;
        this.lazyTag = new LazyTagLongestBalancedSubarrayII();
    }
}

class SegmentTreeLongestBalancedSubarrayII {

    private int n;
    private SegmentTreeNodeLongestBalancedSubarrayII[] tree;

    SegmentTreeLongestBalancedSubarrayII(int[] data) {
        this.n = data.length;
        this.tree = new SegmentTreeNodeLongestBalancedSubarrayII[this.n * 4 + 1];

        for (int i = 0; i < tree.length; i++) {
            tree[i] = new SegmentTreeNodeLongestBalancedSubarrayII();
        }
    ...
    void add(int l, int r, int val) {
        ...
        LazyTagLongestBalancedSubarrayII tag =
                new LazyTagLongestBalancedSubarrayII();
        tag.toAdd = val;
    ...
    private void applyTag(int i, LazyTagLongestBalancedSubarrayII tag) {
